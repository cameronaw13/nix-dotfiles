...
"${nix_editor[@]}" "$templates"/configuration.nix users.users.bootstrap.openssh.authorizedKeys.keys \
...
"${nix_editor[@]}" "$templates"/configuration.nix networking.hostName \
  --val "\"$host_choice\"" --inplace
"${nix_editor[@]}" "$templates"/configuration.nix networking.hostId \
  --val "\"$hostId\"" --inplace
